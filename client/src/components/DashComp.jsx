...
export default function DashComp() {
  ...
  useEffect(() => {
    ...
    const fetchShops = async () => {
        try {
          const res = await fetch('/api/shop/getshops?limit=5');
          const data = await res.json();
          if (res.ok) {
            setShops(data.shops);
            setTotalShops(data.totalShops);
            setLastMonthShops(data.lastMonthShops);
          }
        } catch (error) {
          console.log(error.message);
        }
    ...
    <div className='p-3 md:mx-auto '>
      <div className='flex-wrap flex gap-4 justify-center'>
        ...
        <div className='flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72 w-full rounded-md shadow-md'>
          <div className='flex justify-between'>
            <div className=''>
              <h3 className='text-gray-500 text-md uppercase'>Total Profiles</h3>
        ...
        <div className='flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72 w-full rounded-md shadow-md'>
          <div className='flex justify-between'>
            <div className=''>
              ...
              <p className='text-2xl'>{totalShops || 20}</p>
            </div>
  ...
  <div className="w-full">
    <Chart2/>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 py-3 mx-auto'>
        ...
        <div className='flex flex-col w-full md:w-auto shadow-md p-2 rounded-md dark:bg-gray-800'>
          <div className='flex justify-between  p-3 text-sm font-semibold'>
            <h1 className='text-center p-2'>Recent profiles</h1>
            <Button outline gradientDuoTone='purpleToPink'>
            ...
            shops.map((shop) => (
                <Table.Body key={shop._id} className='divide-y'>
                  <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                    ...
                    <Table.Cell className='w-96'>{shop.title}</Table.Cell>
                    <Table.Cell className='w-5'>{shop.category}</Table.Cell>
                  </Table.Row>
                </Table.Body>
              ))}
          </Table>
